...
# Explicitly force PyTorch to use native SDPA math
...
class DynamicKoopmanEncoder(nn.Module):
    def __init__(self, delay=80, embed_dim=128, top_k=16):
        ...
        # STAGE 1: Basket-Cell Gate (Common-Mode Filter)
    ...
    def forward(self, x, latent_buffer, dream_steps=1, isolate_mode=-1):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # PROBE 2: Modal Isolation
            if isolate_mode != -1 and 0 <= isolate_mode < self.top_k:
                # Zero out all eigenvalues except the requested one
...
class DynamicKoopmanDecoder(nn.Module):
    def __init__(self, top_k=16):
        ...
        # Generates the Biological Skip-Connection Mask
...
class PredictiveVAETrainer:
    ...
    def train_on_transition(self, frame_T, frame_T_plus_1, latent_buffer):
        ...
        with torch.amp.autocast('cuda'):
            # Training loop never isolates modes, always uses -1 (all modes)
            spatial_latent_T, updated_buffer, dynamic_latent_T, _, _ = self.encoder(frame_T, latent_buffer, dream_steps=1, isolate_mode=-1)
...
class LatentVideoFilter:
    def __init__(self, master):
        self.master = master
        self.master.title("Predictive HKT - Biological Probes")
        self.device = device
            ...
            if self.teach_mode and not self.dream_mode and self.latest_frame is not None:
                ...
                if self.frame_T is not None:
                    try:
                        ...
                        loss, self.train_buffer = self.adaptive_trainer.train_on_transition(t_T, t_T1, self.train_buffer)
                        self.status_var.set(f"Training Active | Loss: {loss:.4f}")
                    except Exception:
                        pass
    ...
    def update_video(self):
        ...
        if self.cap is not None and self.cap.isOpened():
            try:
                # 1. Grab appropriate frame input
                ...
                if active_tensor is not None:
                    ...
                    with torch.no_grad():
                        spatial_latent, updated_buffer, dynamic_latent, mag_f, phase_f = self.adaptive_encoder(
                            active_tensor, active_buffer, dream_steps=current_steps, isolate_mode=isolate_val
                        )
                    ...
                    if self.probe3_var.get() and mask_out is not None:
                        # Average the 4 spatial mask channels to 1
                    ...
                    if self.probe1_var.get() and mag_f is not None and phase_f is not None:
                        ...
                        for i in range(len(mags)):
                            ...
                            color = (0, 255, 0) if i == isolate_val else (0, 150, 255)
